...
export class TrailRenderer extends THREE.Object3D {
  ...
  initialize(
    ...
    this.material.uniforms.textureTileFactor.value = new THREE.Vector2(
      1.0,
      1.0
    );
  ...
  initializeLocalHeadGeometry(localHeadWidth, localHeadGeometry) {
    ...
    if (!localHeadGeometry) {
      ...
      for (
        let i = 0;
        i < localHeadGeometry.length && i < TrailRenderer.MaxHeadVertices;
        i++
      ) {
  ...
  initializeGeometry() {
    ...
    const nodeVertexIDs = new Float32Array(
      this.vertexCount * this.VerticesPerNode
    );
    const positions = new Float32Array(
      this.vertexCount * TrailRenderer.PositionComponentCount
    );
    const nodeCenters = new Float32Array(
      this.vertexCount * TrailRenderer.PositionComponentCount
    );
    const uvs = new Float32Array(
      this.vertexCount * TrailRenderer.UVComponentCount
    );
    const indices = new Uint32Array(
      this.faceCount * TrailRenderer.IndicesPerFace
    );
    ...
    const nodeCenterAttribute = new THREE.BufferAttribute(
      nodeCenters,
      TrailRenderer.PositionComponentCount
    );
    ...
    const uvAttribute = new THREE.BufferAttribute(
      uvs,
      TrailRenderer.UVComponentCount
    );
  ...
  pause() {
    if (!this.paused) {
      this.paused = true;
      this.pauseAdvanceUpdateTimeDiff =
        this.currentTime() - this.lastAdvanceTime;
    }
  }
  ...
  resume() {
    if (this.paused) {
      this.paused = false;
      this.lastAdvanceTime =
        this.currentTime() - this.pauseAdvanceUpdateTimeDiff;
    }
  }
  ...
  updateNodeCenter(nodeIndex, nodeCenter) {
    ...
    nodeCenters.updateRange.offset =
      nodeIndex * this.VerticesPerNode * TrailRenderer.PositionComponentCount;
    nodeCenters.updateRange.count =
      this.VerticesPerNode * TrailRenderer.PositionComponentCount;
  }
  ...
  updateNodePositionsFromOrientationTangent = (function () {
    ...
    return function updateNodePositionsFromOrientationTangent(
      ...
      tempQuaternion.setFromUnitVectors(
        TrailRenderer.LocalOrientationTangent,
        orientationTangent
      );
  ...
  connectNodes = (function () {
    ...
    return function connectNodes(srcNodeIndex, destNodeIndex) {
      ...
      returnObj.offset =
        srcNodeIndex * this.FacesPerNode * TrailRenderer.IndicesPerFace;
